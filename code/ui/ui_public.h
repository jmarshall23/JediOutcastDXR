...
// print message on the local console
	...
	int			(*R_Font_StrLenPixels_fn)(const char* text, const int setIndex, const float scale);
	int			(*R_Font_StrLenChars)(const char* text);
	int			(*R_Font_HeightPixels_fn)(const int setIndex, const float scale);
	void		(*R_Font_DrawString_fn)(int ox, int oy, const char* text, const float* rgba, const int setIndex, int iMaxPixelWidth, const float scale);
	...
	inline int R_Font_StrLenPixels(const char* text, const int setIndex, const float scale = 1.0f)
	{
		return R_Font_StrLenPixels_fn(text, setIndex, scale);
	}

	inline int R_Font_HeightPixels(const int setIndex, const float scale = 1.0f)
	{
		return R_Font_HeightPixels_fn(setIndex, scale);
	}

	inline void R_Font_DrawString(int ox, int oy, const char* text, const float* rgba, const int setIndex, int iMaxPixelWidth, const float scale = 1.0f)
	{
		R_Font_DrawString_fn(ox, oy, text, rgba, setIndex, iMaxPixelWidth, scale);
	}
} uiimport_t;

...
{
	NPCInfo->combatMove = qtrue;//always _move straight toward our goal
	...
	//If our _move failed, then reset
	if ( moved == qfalse )
	...
	{
		//First time you successfully _move, say what it is you're doing
...
static qboolean NPC_ST_InvestigateEvent( int eventID, bool extraSuspicious )
{
	...
	*/
	...
	if ( level.alertEvents[eventID].level > AEL_MINOR && NPCInfo->investigateCount > 1 && (NPCInfo->scriptFlags&SCF_CHASE_ENEMIES) )
	{
		...
		{//we were able to _move the investigateGoal to a point in which our bbox would fit
...
void ST_Commander( void )
{
	...
	{//dissolve the group
		...
		for ( i = 0; i < group->numGroup; i++ )
		{
			...
			if ( !(NPCInfo->scriptFlags&SCF_CHASE_ENEMIES) )
			{//not allowed to _move on my own
				continue;
			}
		...
		{//not already retreating
			...
			{//not hit, see if there are other reasons we should run
				...
				{//in the same room as enemy
					...
					{//too close for me to fire my weapon and I'm not already on the _move
		...
		{//okay, we have no new enemy-driven reason to run... let's use tactics now
			...
			{//okay no-one is running, use some tactics
				...
				{//if no-one has a clear shot at the enemy, someone should go after him
					...
					{//only do this if we're ready to _move again and we feel like it
		...
		{//we want to run to a combat point
			...
			*/
			...
			{//found a combat point
				...
				//okay, try a _move right now to see if we can even get there

				//if ( ST_Move() )
				{//we actually can get to it, so okay to say you're going there.
					//FIXME: Hmm... any way we can store this _move info so we don't have to do it again
					//		when our turn to think comes up?

					//set us up so others know we're on the _move
					...
					*/
				}//else: nothing, a failed _move should clear the combatPoint and you can try again next frame
			}
			else if ( NPCInfo->squadState == SQUAD_SCOUT )
...
void NPC_BSST_Attack( void )
{
	...
	{//not supposed to chase my enemies
		if ( NPCInfo->goalEntity == NPC->enemy )
		{//goal is my entity, so don't _move
			_move = qfalse;
		}
	}

...
typedef struct {
	...
	int (*Font_HeightPixels_fn)(const int index, const float scale);
	inline int Font_HeightPixels(const int index, const float scale = 1.0f)
	{
		return Font_HeightPixels_fn(index, scale);
	}

	int (*Font_StrLenPixels_fn)(const char* s, const int index, const float scale);
	inline int Font_StrLenPixels(const char* s, const int index, const float scale = 1.0f)
	{
		return Font_StrLenPixels_fn(s, index, scale);
	}

	int (*Font_StrLenChars_fn)(const char* s);
	inline int Font_StrLenChars(const char* s)
	{
		return Font_StrLenChars_fn(s);
	}

	void (*Font_DrawString_fn)(int x, int y, const char* s, const float* rgba, const int iFontHandle, int iMaxPixelWidth, const float scale);
	...
	{
		Font_DrawString_fn(x, y, s, rgba, iFontHandle, iMaxPixelWidth, scale);
	}

...
typedef void (*roqVQFunction_t)(byte** status, unsigned char* qdata);
...
typedef struct {
	...
	long				samplesPerLine;
	unsigned int		roq_id;
	ptrdiff_t			screenDelta;

	roqVQFunction_t		VQ0;
	roqVQFunction_t		VQ1;
	roqVQFunction_t		VQNormal;
	roqVQFunction_t		VQBuffer;
...
redump:
	...
	case	ROQ_QUAD_VQ:
		if ((cinTable[currentHandle].numQuads & 1)) {
			...
			cinTable[currentHandle].VQ1(cin.qStatus[1], framedata);
			...
			RoQPrepMcomp(cinTable[currentHandle].roqF0, cinTable[currentHandle].roqF1);
			cinTable[currentHandle].VQ0(cin.qStatus[0], framedata);

...
static LRESULT CALLBACK ConWndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_CTLCOLORSTATIC:
        if ((HWND)lParam == s_wcd.hwndBuffer)
        {
            ...
            SetTextColor((HDC)wParam, RGB(249, 249, 0));
...
void Sys_CreateConsole(void)
{
    ...
    //
    s_wcd.hwndInputLine = CreateWindowA(
        "edit",
        NULL,
    ...
    // create the buttons
    //
    s_wcd.hwndButtonCopy = CreateWindowA(
        "button",
        NULL,
    ...
    s_wcd.hwndButtonClear = CreateWindowA(
        "button",
        NULL,
        BS_PUSHBUTTON | WS_VISIBLE | WS_CHILD | BS_DEFPUSHBUTTON,
        82, 425, 72, 24,
    ...
    s_wcd.hwndButtonQuit = CreateWindowA(
        "button",
        NULL,
        BS_PUSHBUTTON | WS_VISIBLE | WS_CHILD | BS_DEFPUSHBUTTON,
    ...
    //
    s_wcd.hwndBuffer = CreateWindowA(
        "edit",
        NULL,
    ...
    // x64-safe subclass install
    s_wcd.SysInputLineWndProc = (WNDPROC)(LONG_PTR)
        SetWindowLongPtr(s_wcd.hwndInputLine, GWLP_WNDPROC, (LONG_PTR)InputLineWndProc);
...
void Sys_ShowConsole(int visLevel, qboolean quitOnClose)
{
    ...
    if (visLevel == s_wcd.visLevel)
    {
        if (quitOnClose)
        {
            // attempt to bring it to the front on error exit
            SetForegroundWindow(s_wcd.hWnd);
            SetFocus(s_wcd.hwndInputLine);
        }
        return;
...
void Sys_SetErrorText(const char* buf)
{
    ...
    if (!s_wcd.hwndErrorBox)
    {
        s_wcd.hwndErrorBox = CreateWindowA(
            "static",
            NULL,
